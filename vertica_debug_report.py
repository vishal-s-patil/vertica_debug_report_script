...
def analyse(query_name, query_result, column_headers, insights_only, with_insights):
    ...
    for threshold in thresholds:
        if threshold['query_name'] == "delete_vectors_count":
            ...
            if ok_count != 0:
                message = "[OK] "
                message += threshold['message_template']['ok'].replace('{val_cnt}', threshold['threshold']['ok'])
                message = message.replace('{cnt}', ok_count)
                print(message)
                
            if wanr_count != 0:
                message = "[WARN] "
                message = threshold['message_template']['warn'].replace('{val_cnt}', threshold['threshold']['warn'])
                message = message.replace('{cnt}', wanr_count)
                print(message)

            if fatal_count != 0:
                message = "[FATAL] "
                message = threshold['message_template']['fatal'].replace('{val_cnt}', threshold['threshold']['fatal'])
